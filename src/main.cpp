...
public:
  PoseEstimator()
  {
    ...
    pcd_sub_ = nh_.subscribe(pcd_topic, 1000, &PoseEstimator::pcdCallback, this);
  ...
  void pcdCallback(const sensor_msgs::PointCloud2ConstPtr& pCloud) {
    ...
    if (ValidPose == true) {
      ...
      if (isnan(X)==false && isnan(Y)==false) {
        ...
        humanPose.pose.position.x = Z; 
        humanPose.pose.position.y = -X; 
...
int main(int argc, char** argv) {
  ...
  nh.getParam("/pcd_topic", pcd_topic);
